...
export const ClientCardComponent = ({ client, onDeleteSuccess, onSaveSuccess }) => {
  ...
  const [editData, setEditData] = useState(() => {
    if (!client) return { name: '', contacts: [''], tag: 'unprocessed', serviceNeeded: '', notes: '', moneyMade: 0 };
    return { ...client, contacts: client.contacts || [''] };
  });
  ...
  const handleSave = async () => {
    ...
    const currentService = isEditUnprocessed ? '' : (editData.serviceNeeded || '').trim();
  ...
  // Animacijos trukmė sumažinta dvigubai (nuo 0.2s/0.15s iki 0.1s/0.07s)
  const fadeVariants = {
  ...
  return (
    <div 
      ref={cardRef}
      className={`relative bg-white dark:bg-[#292a2d] rounded border transition-all duration-300 p-4 shadow-sm group ${isGhosted ? "border-slate-200 dark:border-slate-800 opacity-60 bg-slate-50/50 dark:bg-slate-900/10" : "border-[#dadce0] dark:border-[#3c4043]"} ${isSubmitting ? "opacity-50 pointer-events-none" : ""}`}
      ...
      {!hasRendered ? (
        ...
        <>
          <AnimatePresence mode="wait">
            {isEditing && isAdmin ? (
              <motion.div key="edit-mode" variants={fadeVariants} initial="initial" animate="animate" exit="exit" className="space-y-4">
                <div className="space-y-1.5">
                  <label className="label-base"><MdPerson size={14} className="text-[#1a73e8]" /> Kliento pavadinimas</label>
                  <input type="text" className="input-base" value={editData.name || ''} onChange={(e) => setEditData({...editData, name: e.target.value})} />
                </div>

                <div className="space-y-1.5">
                  <label className="label-base"><MdContactMail size={14} className="text-[#1a73e8]" /> Kontaktai</label>
                  <div className="space-y-2">
                    {(editData.contacts || []).map((contact, index) => (
                      <div key={index} className="flex gap-2 items-start">
                        <input type="text" className="input-base pl-9" value={contact} onChange={(e) => handleContactChange(index, e.target.value)} />
                ...
                <div className="space-y-1.5">
                  <label className="label-base"><MdBadge size={14} className="text-[#1a73e8]" /> Kliento žymė</label>
                  <select className={`input-base capitalize ${CLIENT_TAGS_CONFIG[editData.tag]?.colorClass || ''}`} value={editData.tag} onChange={(e) => handleTagChange(e.target.value)}>
                    {ALL_TAGS.map(t => (
                ...
                {showMoney && (
                  <div className="space-y-1.5">
                    <label className="label-base"><MdEuro size={14} className="text-[#1a73e8]" /> Uždirbta suma</label>
                    <div className="relative">
                      <MdEuro className="absolute left-2.5 top-2.5 text-slate-500" size={16} />
                      <input type="number" className="input-base pl-9" value={editData.moneyMade || 0} onChange={(e) => setEditData({...editData, moneyMade: parseFloat(e.target.value) || 0})} />
                    </div>
                  </div>
                )}
                ...
                <div className={`space-y-1.5 ${isServiceInputGhosted ? "opacity-60" : "opacity-100"}`}>
                  ...
                  <input 
                    type="text" 
                    disabled={isServiceInputGhosted || !isAdmin} 
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea className="input-base resize-none h-auto" rows="3" value={editData.notes || ''} onChange={(e) => setEditData({...editData, notes: e.target.value})} />
                </div>
                ...
                <div className="flex justify-between items-start gap-2">
                  ...
                  {isAdmin && (
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                      <button onClick={() => setIsEditing(true)} className="btn-blue-icon"><MdEdit size={16} /></button>
